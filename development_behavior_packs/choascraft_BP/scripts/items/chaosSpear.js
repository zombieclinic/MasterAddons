...
function ensureSpearLoop() {
	if (spearLoopRunning || activeSpears.size === 0) return;
	spearLoopRunning = true;

	const update = () => {
		tickSpears();
		if (activeSpears.size > 0) {
			system.runTimeout(update, 1);
		} else {
			spearLoopRunning = false;
		}
	};
